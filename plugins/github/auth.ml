...
let login t : Current_web.Resource.t = object
  method get_raw site request =
    ...
    | Some t ->
      ...
      | None, _ -> 
        Server.respond_error ~status:`Bad_request ~body:"Missing code" () >|= fun r -> `Response r
      | _, None -> Server.respond_error ~status:`Bad_request ~body:"Missing state" () >|= fun r -> `Response r
      | Some code, Some state ->
        if state <> Current_web.Context.csrf ctx then (
          Server.respond_error ~status:`Bad_request ~body:"Bad CSRF token" () >|= fun r -> `Response r
        ) else (
          ...
          | Error (status, msg) ->
            ...
            Server.respond_error ~status:`Internal_server_error ~body:"Failed to get token" () >|= fun r -> `Response r
          | Ok token ->
            ...
            | Error (status, msg) ->
              ...
              Server.respond_error ~status:`Internal_server_error ~body:"Failed to get user details" () >|= fun r -> `Response r
            | Ok user ->
              Log.info (fun f -> f "Successful login for %S" user);
              match Current_web.User.v user with
              | Error (`Msg m) ->
                Log.warn (fun f -> f "Failed to create user: %s" m);
                Server.respond_error ~status:`Bad_request ~body:"Bad user" () >|= fun r -> `Response r
              | Ok user ->
                Current_web.Context.set_user ctx user
        )

  method post_raw _ _ _ =
    Server.respond_error ~status:`Bad_request ~body:"Bad method" () >|= fun r -> `Response r
