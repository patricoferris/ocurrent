...
type t = build * Eio.Fs.dir Eio.Path.t
...
let with_context ~fs ~job context fn =
  match context with
  | `No_context -> Current.Process.with_tmpdir ~prefix:"build-context-" fs fn
  | `Dir path ->
      Current.Process.with_tmpdir ~prefix:"build-context-" fs @@ fun dir ->
      Current.Process.exec ~cwd:(Fpath.v @@ snd dir) ~cancellable:true ~job ("", [ "rsync"; "-aHq"; Fpath.to_string path ^ "/"; "." ]) >>?= fun () ->
      fn dir
  | `Git commit -> Current_git.with_checkout ~job commit fn

let build ({ pull; pool; timeout; level }, fs) job key =
  ...
  with_context ~job ~fs commit @@ fun dir ->
