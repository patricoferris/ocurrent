...
type t = (auth option * Eio.Fs.dir Eio.Path.t)
...
let publish (auth, fs) job tag value =
  ...
  match Current.Process.exec ~cancellable:true ~job (create_cmd ~config ~tag value) with
  | Error _ as e -> e
  | Ok () ->
