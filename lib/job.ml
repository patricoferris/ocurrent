...
let create ?(priority=`Low) ~sw ~switch ~label ~config () =
  ...
  | Ok (_ : bool) ->
    ...
    let t = { sw; switch; id; path = Some path; start_time; set_start_time; config; log_cond; cancel_hooks;
...
let use_pool ?(priority=`Low) ~switch t pool =
  let th, cancel = Pool.get ~sw:t.sw ~priority ~switch pool in
  on_cancel t (fun _ -> cancel ());
  th

let no_pool =
  Pool.of_fn ~label:"no pool" (fun ~sw:_ ~priority:_ ~switch:_ -> Eio.Promise.create_resolved (Ok ()), fun () -> ())
