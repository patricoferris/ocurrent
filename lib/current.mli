...
module Switch : sig
  ...
  val turn_off : t -> unit Eio.Promise.t
...
module Pool : sig
  ...
  val of_fn :
    label : string ->
    (sw:Eio.Switch.t -> priority:priority -> switch:Switch.t -> 'a Eio.Promise.or_exn * (unit -> unit)) ->
    'a t
...
module Job : sig
  ...
  val use_pool : ?priority:Pool.priority -> switch:Switch.t -> t -> 'a Pool.t -> 'a Eio.Promise.or_exn
